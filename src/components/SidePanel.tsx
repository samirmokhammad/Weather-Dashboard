...
export default function SidePanel(props: Props) {
  ...
  return (
    <div
      className={clsx(
        'fixed top-0 right-0  h-screen w-(--sidebar-width) shadow-md bg-sidebar z-1002 py-8 px-4 overflow-y-scroll transition-transform duration-300 lg:translate-x-0!',
        isSidePanelOpen ? 'translate-x-0' : 'translate-x-full',
      )}
    >
      <button onClick={() => setIsSidePanelOpen(false)}>
        <Chevron className="size-8 invert -ml-2 cursor-pointer lg:hidden" />
      </button>
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {Object.entries(data.list[0].components).map(([key, value]) => {
        const pollutant = airQualityRanges[key.toUpperCase() as Pollutant];
        ...
        const currentLevel = (() => {
          ...
          return 'Very Poor' as AirQualityLevel;
        })();
        const qualityColor = (() => {
          switch (currentLevel) {
            case 'Good':
